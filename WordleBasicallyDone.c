...
int compareWords(const char* targetWord, const char* userGuess) {
    ...
    for (int i = 0; i < WORD_LENGTH; i++) {
        if (tempTarget[i] == tempGuess[i]) {
            ...
            tempTarget[i] = ' '; // Mark as seen in target word
            tempGuess[i] = '*';  // Mark as seen in guess word
        }
    }

    // Gets correct characters in wrong positions
    for (int i = 0; i < WORD_LENGTH; i++) {
        for (int j = 0; j < WORD_LENGTH; j++) {
            if (tempTarget[i] != ' ' && tempTarget[i] == tempGuess[j] && tempGuess[j] != '*') {
                correctCharacterCount++;
                printf("%c is in the word but in the wrong position\n", userGuess[j]); // Print the correct letter
                tempTarget[i] = ' '; // Mark as seen in target word
                tempGuess[j] = '*';  // Mark as seen visited in guess word
                break;
            }
        }
    }
        ...
        for (int guessCount = 0; guessCount < 6; guessCount++) {
            ...
            if (isWordInList("wordlist.txt", userGuess)) {
                ...
                if (result == 1) {
                    ...
                    break; // Exit loop if word is guessed correctly
                } else {
